...
plot(myTableTry1(23:end-2,2)/max(myTableTry1(:,2)),myTableTry1(23:end-2,1)/max(myTableTry1(:,1)),"o-",lw,1.5); hold on
plot(myTableTry2(23:end-2,2)/max(myTableTry2(:,2)),myTableTry2(23:end-2,1)/max(myTableTry2(:,1)),"o-",lw,1.5);
plot(myTableTry3(23:end-2,2)/max(myTableTry3(:,2)),myTableTry3(23:end-2,1)/max(myTableTry3(:,1)),"o-",lw,1.5);
plot(myTableTry4(23:end-2,2)/max(myTableTry4(:,2)),myTableTry4(23:end-2,1)/max(myTableTry4(:,1)),"o-",lw,1.5);
...
%% Standar Error Plot
dataSet = [myTableTry1(23:end-2,1) myTableTry2(23:end-2,1) myTableTry3(23:end-2,1) myTableTry4(23:end-2,1) myTableTry5(23:end-2,1)];
stdError = zeros(length(dataSet),1);
for i = 1:length(dataSet)
    stdError(i) = std(dataSet(i,:)/max(dataSet(i,:)))/sqrt(length(dataSet(i,:)));
end
xAxis = myTableTry2(23:end-2,2)/max(myTableTry1(:,2));
