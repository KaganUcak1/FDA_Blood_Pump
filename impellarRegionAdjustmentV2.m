...
% Last edit: 20.04.2023, Created 26.03.2023,
clear; clc; close all
...
for k = ["A", "B", "C", "D", "E", "F"]
    ...
    elseif z == 3
        xShift= -1; yShift = -16; axisArray = [-5 30 -27 10];
    elseif z == 6
        xShift= -1.25; yShift = 7.5; axisArray = [-5 30 -13 25];
    elseif z == 5
        xShift= -.5; yShift = 0; axisArray = [-5 30 -13 25];
    else
    ...
    contourf(x2(1:end)+xShift,-y2(1:end)+yShift,Z,300,"LineColor","none")
    max(z2)
    axis equal
    ...
    hold on;
    quiver(newTable(1:arrowDensity:end,1),newTable(1:arrowDensity:end,2),newTable(1:arrowDensity:end,3),newTable(1:arrowDensity:end,4),1.5,"k","LineWidth",1);
    axis equal;axis(axisArray)
    title("Angle " +k);
    pause(2);
    saveas(f2,".\contourPlots\contourAngle"+k+".png");
    pause(2);
